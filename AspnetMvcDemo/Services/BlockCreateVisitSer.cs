...
{
    public class BlockCreateVisitSer
    {
        ...
        public void CreateVisit()
        {
            ...
            Users = userServ.BlockUserDetails();
            while (Count < 30)
            {
                queryResult = visitServ.GetBlockVisits();
        ...
        private void autoCreate(DateTime dateTime, int Count)
        {
            ...
            {
                //check the last factid for this user in BlockVisitDetails 


                var lastfact = entities.BlockVisitDetails.ToList().LastOrDefault();
                var totalvisitdate = (from vs in entities.BlockVisitDetails
                                      join f in entities.BlockFactories
                ...
                var Mid = (from vs in entities.BlockVisitDetails
                           join fac in entities. BlockFactories
                ...
                var factid = (from vs in entities.BlockVisitDetails
                              join f in entities. BlockFactories
                              on vs.FactoryId equals f.Id
                        ...
                        if (totalvisitdate.Count != 0)
                        {
                            ...
                            else
                            {
                                ...
                                else if (Mid == Users[1].Id)
                                {
                                    ...
                                    var totalvisits2User = (from vs in entities.BlockVisitDetails
                                                            join f in entities. BlockFactories
                                                            on vs.FactoryId equals f.Id
                                ...
                                {
                                    //retives all visits for this user in this location
                                    var totalvisits3User = (from vs in entities.BlockVisitDetails
                                                            join f in entities. BlockFactories
                                                            on vs.FactoryId equals f.Id
                                                            where f.Location_Id == user.Location_Id
                                                            && vs.MonitorId == Mid
                    ...
                    if (user == Users[2])
                    {
                        if (totalvisitdate.Count != 0)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    //retives all visits for this user in this location
                                    var totalvisits5User = (from vs in entities.BlockVisitDetails
                                                            join f in entities. BlockFactories
                                                            on vs.FactoryId equals f.Id
                                                            where f.Location_Id == user.Location_Id
                                                            && vs.MonitorId == Mid
                                ...
                                {
                                    //retives all visits for this user in this location
                                    var totalvisits6User = (from vs in entities.BlockVisitDetails
                                                            join f in entities. BlockFactories
                                                            on vs.FactoryId equals f.Id
                                                            where f.Location_Id == user.Location_Id
                                                            && vs.MonitorId == Mid
        ...
        public void AssignnewVisit(BlockUser user, DateTime date)
        {
            var Count = (from count in entities.BlockVisitDetails
                         join f in entities. BlockFactories
                         on count.FactoryId equals f.Id
                         where f.Location_Id == user.Location_Id
                         select count.Id).ToList().Count;
            var totalfact = entities. BlockFactories.Where(f => f.Location_Id == user.Location_Id && f.IsActive == true).ToList().Count;
             BlockFactory[] fact;
            if (Count == totalfact)
            {
                fact = entities. BlockFactories.Where(f => f.Location_Id == user.Location_Id && f.IsActive == true).OrderBy(f => f.Id).ToArray();
            }
            else
            {
                fact = entities. BlockFactories.Where(f => f.Location_Id == user.Location_Id && f.IsActive == true).OrderBy(f => f.Id).Skip(Count).ToArray();
            ...
            if (fact.Length > 0)
            {
                result1 = visitServ.CreateBlockVisits(new BlockVisitDetail
                {
                    FactoryId = fact[0].Id,
                    MonitorId = user.Id,
                    VisitDate = date
                ...
                {
                    result2 = visitServ.CreateBlockVisits(new BlockVisitDetail
                    {
                        FactoryId = fact[1].Id,
                        MonitorId = user.Id,
                        VisitDate = date
        ...
        public void AssignIncrementVisit(int Mid, int factid, int? locationid, DateTime date)
        {
            ...
            var totalfact = entities. BlockFactories.Where(f => f.Location_Id == locationid && f.IsActive == true).ToList().Count;
             BlockFactory[] fact;
            if (Count == totalfact)
            {
                fact = entities. BlockFactories.Where(f => f.Location_Id == locationid && f.IsActive == true).OrderBy(f => f.Id).ToArray();
            }
            else
            {
                fact = entities. BlockFactories.Where(f => f.Location_Id == locationid && f.IsActive == true).OrderBy(f => f.Id).Skip(Count).ToArray();
